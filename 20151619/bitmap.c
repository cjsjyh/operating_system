...
{
  int prev_size = b->bit_cnt;
  if (b != NULL)
    {
      b->bit_cnt = prev_size + size;
      b->bits = realloc (b->bits, byte_cnt (prev_size + size));
      if (b->bits != NULL || size == 0)
        {
          bitmap_set_multiple (b, prev_size, size, 0);
          return b;
        }
      free (b);
    }
